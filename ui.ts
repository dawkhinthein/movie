export function renderWebsite() {
    ...
    <style>
      ...
      * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
      ...
      .player-overlay { 
          ...
          /* IMPORTANT: Pointer events none allows click-through to video, but children need auto */
          pointer-events: none; 
          background: linear-gradient(to bottom, rgba(0,0,0,0.7) 0%, transparent 20%, transparent 80%, rgba(0,0,0,0.7) 100%); 
      }
      
      /* 🔥 HIDDEN STATE (Fade out completely) */
      .player-overlay.hidden { opacity: 0; }

      /* Buttons inside overlay must be clickable */
  ...
  <body>
    ...
    <div id="userPanel" class="user-panel">
        ...
        <div id="loginForm"><input type="text" id="reg_user" class="auth-input" placeholder="Username"><input type="password" id="reg_pass" class="auth-input" placeholder="Password"><div style="margin:10px 0; display:flex; align-items:center;"><input type="checkbox" id="rememberMe" style="width:auto; margin-right:8px;"><label for="rememberMe" style="color:#aaa; font-size:13px;">Remember me (15 Days)</label></div><button class="auth-btn" onclick="doLogin()">Login</button><button class="auth-btn secondary" onclick="doRegister()">Register New</button></div>
        <div id="profileView" style="display:none;"><p style="color:#aaa; font-size:13px;">Logged in as:</p><h3 id="u_name" style="color:#e50914; margin-top:0;">User</h3><div style="background:#222; padding:10px; border-radius:6px; margin:10px 0;"><p style="margin:0; font-size:12px; color:#aaa;">Status</p><p id="u_status" style="margin:5px 0; font-weight:bold;">Free Plan</p></div><p style="margin-bottom:5px; font-size:13px;">Redeem Code:</p><div style="display:flex; gap:5px;"><input type="text" id="vip_code" class="auth-input" style="margin:0;" placeholder="Code"><button class="auth-btn" style="margin:0; width:auto;" onclick="doRedeem()">Go</button></div><button class="auth-btn secondary" style="margin-top:20px;" onclick="openFavorites(); toggleUserPanel();">❤️ My Favorites</button><button class="auth-btn secondary" onclick="doLogout()">Log Out</button></div>
    ...
    <div id="playerModal">
      <div class="modal-content">
        <div class="video-area" id="videoWrapper">
            ...
            <div id="vip-lock"><div style="font-size:40px; margin-bottom:10px;">👑</div><h2>Premium Content</h2><p>This content requires a VIP subscription.<br>Please login or redeem a code.</p><button class="lock-btn" onclick="closePlayer(); toggleUserPanel();">Login / Redeem</button></div>
            ...
            <div class="player-overlay" id="playerOverlay">
                ...
                <div class="bottom-controls"><select id="qualitySelect" class="quality-select" style="display:none;" onchange="changeQuality(this)"></select><button class="ctrl-btn" onclick="toggleFullScreen()">⛶</button></div>
            </div>
        </div>
        <div class="info-sec"><h2 id="m_title"></h2><div style="margin:10px 0" id="m_tags"></div><div class="action-row"><button id="favBtn" class="fav-btn" onclick="toggleFavorite()">🤍 Add to Fav</button><div id="dl_area"></div></div><p id="m_desc" class="desc"></p><div id="ep_section" style="margin-top:20px;"></div></div>
      </div>
    </div>
    ...
    <script>
      ...
      window.onload = async () => {
        loadSession(); updateProfileUI(); await loadHomeData(); setupPlayerIdle(); hideLoader();
        const urlParams = new URLSearchParams(window.location.search);
        const movieId = urlParams.get('id');
        const view = urlParams.get('view');
      ...
      function createCardHtml(m) { const tag = m.isPremium ? '<div class="prem-tag">👑</div>' : ''; return \`<div class="card" onclick="openModalById('\${m.id}')"><img src="\${m.image}" loading="lazy" onerror="this.src='https://via.placeholder.com/150x225?text=No+Img'" oncontextmenu="return false;">\${tag}<div class="title">\${m.title}</div></div>\`; }
      ...
      function openCategory(cat, pushState = true) { showLoader(); currentCategory = cat; showGridInternal(); document.getElementById('gridTitle').innerText = decodeURIComponent(cat).toUpperCase(); if(pushState) { const u = \`?view=grid&cat=\${encodeURIComponent(cat)}\`; window.history.pushState({path:u},'',u); } fetchMovies(1, cat).then(hideLoader); }
      async function openFavorites() { showLoader(); showGridInternal(); document.getElementById('gridTitle').innerText = "MY FAVORITES"; document.getElementById('mainGrid').innerHTML = getClientSkeleton(6); document.getElementById('pagControls').style.display = 'none'; window.history.pushState({},'','?view=grid&cat=fav'); const favs = JSON.parse(localStorage.getItem('my_favs') || '[]'); if(favs.length === 0) { document.getElementById('mainGrid').innerHTML = '<p style="grid-column:1/-1; text-align:center;">No favorites yet.</p>'; hideLoader(); return; } let html = ''; for(const id of favs) { try { const res = await fetch(\`/api/get_movie?id=\${id}\`); const m = await res.json(); if(m && m.title) { html += createCardHtml(m); if(!allMoviesData.find(x=>x.id===m.id)) allMoviesData.push(m); } } catch(e){} } document.getElementById('mainGrid').innerHTML = html; hideLoader(); }
      async function executeSearch() { const q = document.getElementById('searchInput').value; if(!q) return goHome(); showLoader(); showGridInternal(); document.getElementById('gridTitle').innerText = "SEARCH: " + q; window.history.pushState({},'','?view=grid&q='+encodeURIComponent(q)); document.getElementById('mainGrid').innerHTML = getClientSkeleton(10); document.getElementById('pagControls').style.display = 'none'; const res = await fetch(\`/api/search?q=\${encodeURIComponent(q)}\`); const results = await res.json(); allMoviesData = results; renderGrid(results); hideLoader(); }
      function handleSearchKey(e) { if(e.key==='Enter') executeSearch(); }
      async function fetchMovies(page, cat) { document.getElementById('mainGrid').innerHTML = getClientSkeleton(10); const encodedCat = (cat==='all'||cat==='movies'||cat==='series') ? cat : encodeURIComponent(cat); const res = await fetch(\`/api/movies?page=\${page}&cat=\${encodedCat}\`); const json = await res.json(); allMoviesData = json.data; renderGrid(json.data); const div = document.getElementById('pagControls'); if(json.totalPages > 1) { div.style.display = 'flex'; document.getElementById('prevBtn').disabled = !json.hasPrev; document.getElementById('nextBtn').disabled = !json.hasNext; document.getElementById('pageInfo').innerText = \`Page \${json.currentPage}\`; } else div.style.display = 'none'; currentPage = json.currentPage; }
      function renderGrid(data) { const grid = document.getElementById('mainGrid'); if(!data || data.length === 0) grid.innerHTML = '<p style="grid-column:1/-1; text-align:center;">Empty</p>'; else grid.innerHTML = data.map(m => createCardHtml(m)).join(''); }

      // --- PLAYER ---
      // 🔥 UPDATED: Fix Auto-Hide Logic
      function setupPlayerIdle() {
          ...
          function show() {
              ...
              if(!v.paused) {
                  controlsTimeout = setTimeout(() => o.classList.add('hidden'), 3000);
              }
          }
          
          w.addEventListener('mousemove', show);
          w.addEventListener('touchstart', show);
      ...
      window.startPlayback = function() {
          ...
          const vid = document.getElementById('video');
          playViaSecureToken(activeVideoLink).then(() => { const t = localStorage.getItem('watch_'+currentMovieId); if(t) vid.currentTime = parseFloat(t); });
      }
