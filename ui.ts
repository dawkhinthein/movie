  ...
  <body>
    ...
    <div id="custom-alert" style="display:none;">
        <div class="alert-box">
            ...
            <p id="alert-msg" class="alert-msg">Operation completed.</p>
    ...
    <div id="userPanel" class="user-panel">
        ...
        <div id="loginForm">
            ...
            <div style="margin:10px 0; display:flex; align-items:center;">
                ...
                <label for="rememberMe" style="color:#aaa; font-size:13px;">Remember me (15 Days)</label>
            </div>
        ...
        <div style="margin-top:auto; padding-top:15px; border-top:1px solid #333; text-align:center;">
            <p style="color:#aaa; font-size:12px; margin-bottom:10px;">Need Help or VIP Code?</p>
            <a href="https://t.me/iqowoq" target="_blank" class="auth-btn" style="background:#0088cc; text-decoration:none; display:block;">✈️ Contact Admin</a>
        </div>
    </div>
    ...
    <script>
      ...
      function loadSession() {
          ...
          if (session.sessionExpiry && Date.now() > session.sessionExpiry) { localStorage.removeItem('user_session'); return; }
          currentUser = session;
      }
      ...
      async function doRegister() {
          ...
          if(!u || !p) return showAlert("Error", "Missing fields", false);
          showLoader();
          const res = await fetch('/api/auth/register', { method:'POST', body:JSON.stringify({username:u, password:p}) });
          hideLoader();
          if(res.ok) showAlert("Success", "Registered! Please Login.");
          else showAlert("Error", "Username taken", false);
      }
      ...
      async function doLogin() {
          ...
          if(res.ok) {
              ...
              user.sessionExpiry = Date.now() + (remember ? 15 : 1) * 24 * 60 * 60 * 1000;
              currentUser = user;
              localStorage.setItem('user_session', JSON.stringify(currentUser));
              updateProfileUI();
              showAlert("Welcome", "Login successful!");
          } else showAlert("Error", "Invalid credentials", false);
      }
      ...
      // 🔥 CARD HTML WITH YEAR TAG FIX
      function createCardHtml(m) { 
          const tag = m.isPremium ? '<div class="prem-tag">👑</div>' : ''; 
          // Find year in tags (4 digits)
