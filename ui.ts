  ...
  <body>
    ...
    <script>
      ...
      window.addEventListener('popstate', function(event) {
        ...
        if (movieId) {
             // If navigating forward to a movie from history, ensure modal open
             openModalById(movieId);
        } else if (view === 'grid') {
             // If navigating back to grid
             closePlayerInternal();
             // Reload grid if empty (important for deep linking)
      ...
      // 🔥 FIX: INFINITE SCROLL RESET LOGIC
      function resetGridState() {
          currentPage = 1;
          isLoading = false;
          hasMore = true;
          allMoviesData = []; // Clear local data cache
      ...
      function openCategory(c,p=true){
          showLoader(); currentCategory=c; showGridInternal(); 
      ...
      async function executeSearch(){
          const q=document.getElementById('searchInput').value;if(!q)return goHome();
          showLoader(); showGridInternal(); 
          ...
          try{const res=await fetch(\`/api/search?q=\${encodeURIComponent(q)}\`);const results=await res.json();allMoviesData=results;if(results.length===0)document.getElementById('mainGrid').innerHTML='<p style="grid-column:1/-1;text-align:center;padding:20px;">No results found.</p>';else renderGrid(results,false);}catch(e){document.getElementById('mainGrid').innerHTML='<p style="grid-column:1/-1;text-align:center;">Error.</p>';}finally{hideLoader();}
      }
